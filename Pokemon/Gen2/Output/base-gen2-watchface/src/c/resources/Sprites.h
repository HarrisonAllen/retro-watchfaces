...
static uint16_t load_Ethan~bw(GBC_Graphics *gbc_graphics, uint8_t vram_bank, uint8_t vram_start_offset, uint8_t palette_num, uint8_t *num_sprites, const uint8_t **sprite_data) {
    // Load Ethan~bw tilesheet
    uint16_t num_loaded_tiles = load_tilesheet(gbc_graphics, RESOURCE_ID_DATA_ETHAN~BW_TILESHEET, 0, vram_start_offset, vram_bank);

    // Load Ethan~bw palette
    GBC_Graphics_set_sprite_palette_array(gbc_graphics, palette_num, ETHAN~BW_PALETTES[0]);

    // Set number of sprites
    *num_sprites = 4;

    // Set sprite data
    *sprite_data = ETHAN~BW_DATA;
...
static uint16_t load_Kris~bw(GBC_Graphics *gbc_graphics, uint8_t vram_bank, uint8_t vram_start_offset, uint8_t palette_num, uint8_t *num_sprites, const uint8_t **sprite_data) {
    // Load Kris~bw tilesheet
    uint16_t num_loaded_tiles = load_tilesheet(gbc_graphics, RESOURCE_ID_DATA_KRIS~BW_TILESHEET, 0, vram_start_offset, vram_bank);

    // Load Kris~bw palette
    GBC_Graphics_set_sprite_palette_array(gbc_graphics, palette_num, KRIS~BW_PALETTES[0]);

    // Set number of sprites
    *num_sprites = 4;

    // Set sprite data
    *sprite_data = KRIS~BW_DATA;
