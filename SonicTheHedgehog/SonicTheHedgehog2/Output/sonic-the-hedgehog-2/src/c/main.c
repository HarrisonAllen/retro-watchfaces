...
static void load_background_palettes(uint8_t *palettes[]) {
    for (uint8_t p = 0; p < GBC_PALETTE_NUM_PALETTES; p++) {
        GBC_Graphics_set_bg_palette_array(s_gbc_graphics, p, palettes[p]);
        GBC_Graphics_set_sprite_palette_array(s_gbc_graphics, p, SPRITE_PALETTES[p]);
    }
}

static void load_sprite_palettes(uint8_t *palettes[]) {
    for (uint8_t p = 0; p < GBC_PALETTE_NUM_PALETTES; p++) {
        GBC_Graphics_set_sprite_palette_array(s_gbc_graphics, p, palettes[p]);
    }
}
...
static void load_tilemap(uint32 map_resource_id, )
